...
parser = argparse.ArgumentParser(description='Image Denoising using TSP_RDANet')
...
with torch.no_grad():
    for i in tqdm(range(40)):
        for k in range(32):
            ...
            restored_patch_all = torch.clamp(restored_patch[1], 0, 1).cpu().detach().permute(0, 2, 3, 1).squeeze(0)
            restored[i, k, :, :, :] = restored_patch_all

            if args.save_images:
                save_file = os.path.join(result_dir_img, '%04d_%02d.png' % (i+1, k+1))
                utils.save_img(save_file, img_as_ubyte(restored_patch[1]))
