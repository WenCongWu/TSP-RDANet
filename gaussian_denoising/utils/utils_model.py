...
def test_onesplit(model, L, refield=32, min_size=256, sf=1, modulo=1):
    ...
    b, c = Es[0][0].size()[:2]
    E = torch.zeros(b, c, sf * h, sf * w).type_as(L)
    E[..., :h // 2 * sf, :w // 2 * sf] = Es[0][0][..., :h // 2 * sf, :w // 2 * sf]
    E[..., :h // 2 * sf, w // 2 * sf:w * sf] = Es[1][0][..., :h // 2 * sf, (-w + w // 2) * sf:]
    E[..., h // 2 * sf:h * sf, :w // 2 * sf] = Es[2][0][..., (-h + h // 2) * sf:, :w // 2 * sf]
    E[..., h // 2 * sf:h * sf, w // 2 * sf:w * sf] = Es[3][0][..., (-h + h // 2) * sf:, (-w + w // 2) * sf:]
