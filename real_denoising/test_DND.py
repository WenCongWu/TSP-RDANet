...
with torch.no_grad():
    for i in tqdm(range(50)):
        ...
        for k in range(20):
            ...
            restored_patch_all = torch.clamp(restored_patch[1], 0, 1).cpu().detach().permute(0, 2, 3, 1).squeeze(0).numpy()
            Idenoised[k] = restored_patch_all

            if args.save_images:
                save_file = os.path.join(result_dir_img, '%04d_%02d.png' % (i+1, k+1))
                denoised_img = img_as_ubyte(restored_patch[1])
